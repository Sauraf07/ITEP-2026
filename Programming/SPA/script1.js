...
function loginUser(){
    let name = prompt("Enter your name to sign in:");
    if(name && name.trim() !== ""){
...
function searchProducts(event){
    ...
    renderFilteredCards(filteredProducts);
}
function renderFilteredCards(products){
   let main = document.querySelector("#main");

   let old = document.querySelector("#card-container");
   ...
   for(let product of products){
     ...
     title.innerText = product.title.substring(0,30);

     let price = document.createElement("h5");
     price.innerHTML = `<span class="text-success">${product.price} Rs</span>`;

     let btn = document.createElement("button");
     btn.className = "btn btn-warning w-75";
     btn.innerText = "View More";
     btn.onclick = () => ViewMoreComponent(product);
...
function HeaderComponent(){
    ...
    header.className = "bg-dark d-flex justify-content-between align-items-center px-3";
    header.style.height = "60px";

    let logo = document.createElement("div");
    logo.innerHTML = "<b class='text-white'><span class='text-danger'>E-shop</span></b>";

    let search = document.createElement("input");
    search.className = "form-control w-50";
...
function ViewMoreComponent(product){
   ...
   if(cards) main.removeChild(cards);

   let oldCart = main.querySelector(".cart-page");
   if(oldCart) main.removeChild(oldCart);
   ...
   dec.innerHTML = `<strong>Description: </strong>${product.description}`;
   right.appendChild(dec);

   let productcatagory = document.createElement("h6");
   productcatagory.innerHTML = `<strong>Catogory: </strong>${product.category}`;
   right.appendChild(productcatagory);

    let price = document.createElement("h6");
   price.innerHTML = `<strong>Price: </strong>${product.price}`;
   right.appendChild(price);

    let disc = document.createElement("h6");
   disc.innerHTML = `<strong>Discount: </strong>${product.discountPercentage}%`;
   right.appendChild(disc);

    let waranty = document.createElement("h6");
   waranty.innerHTML = `<strong>Waranty : </strong>${product.warrantyInformation}`;
   right.appendChild(waranty);

   let ratingTitle = document.createElement("p");
    ratingTitle.innerHTML = `<strong>Rating:  </strong>${product.rating}`;
    right.appendChild(ratingTitle);

    let stockTitle = document.createElement("p");
    stockTitle.innerHTML = ` <strong>Stock:  </strong> ${product.stock}`;
    right.appendChild(stockTitle);

    let brandTitle = document.createElement("p");
    brandTitle.innerHTML = ` <strong>Brand: </strong> ${product.brand}`;
    right.appendChild(brandTitle);

    let skuTitle = document.createElement("p");
    skuTitle.innerHTML = `<strong>ID:  </strong> ${product.sku}`;
    right.appendChild(skuTitle);

    let reviews = document.createElement("p");
    reviews.innerHTML= `<strong>Rating: </strong> ${product.rating}`;
   ...
   backBtn.className = "btn btn-secondary mt-3 ms-2 ++--";
   backBtn.innerText = "Back";
   backBtn.onclick = CardComponent;

   right.append(cartBtn, backBtn);
...
function CartComponent(){
    ...
    let oldView = main.querySelector(".container.mt-3");
    if(oldView) main.removeChild(oldView);

    let container = document.createElement("div");
    container.className = "container mt-3 cart-page";
    ...
    container.append(title, backBtn);
    ...
    for(let i = 0; i < cart.length; i++){
        ...
        let row = document.createElement("div");
        row.className = "d-flex align-items-center justify-content-between border p-2 mb-2";
        ...
        row.append(img, name, price, removeBtn);
        container.appendChild(row);
    }
