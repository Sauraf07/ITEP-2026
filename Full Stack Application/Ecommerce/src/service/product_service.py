...
class ProductService:
    ...
    async def create(
        ...
        print(
            f"{title} : {price} : {description} : {warranty_information} : {rating} : {product_image} : {category_id}"
        )
        ...
        product = Product(
            ...
            product_image="/public/images/" + product_image.filename,
        )
    ...
    async def fetch_by_id(self, id: int):
        return await self.product_repo.fetch_by_id(id)
