import Nav from "../nav/Nav";
import Api, { BASE_URL } from "../../Api";
import { useNavigate } from "react-router-dom";
import { useEffect, useReducer } from "react";
import axios from "axios";

function Home() {
  const [state, dispatch] = useReducer(
    (state, action) => {
      if (action.type == "set-categories") state.categories = action.payload;
      else if (action.type == "set-products") state.products = action.payload;
      return { ...state };
    },
    {
      categories: [],
      products: [],
    },
  );
  useEffect(() => {
    const loadStorefront = async () => {
      try {
        const [categories, products] = await Promise.all([
          axios.get(Api.FETCH_CATEGORIES),
          axios.get(Api.FETCH_PRODUCTS),
        ]);
        dispatch({ type: "set-categories", payload: categories.data });
        dispatch({ type: "set-products", payload: products.data });
      } catch (error) {
        console.error("Unable to load storefront data", error);
      }
    };

    loadStorefront();
  }, []);
  const navigate = useNavigate();
  return (
    <>
      <Nav />
      <div className="container mt-3">
        <div className="row">
          {state.categories.map((category) => {
            return (
              <div key={category.id} className="col-md-2">
                <div
                  className="category-container border d-flex justify-content-around align-items-center"
                  style={{ height: "60px" }}
                >
                  <img
                    src={BASE_URL + category.category_image}
                    width="50px"
                    height="50px"
                  />
                  <label style={{ fontSize: "12px" }}>
                    {category.category_name}
                  </label>
                </div>
              </div>
            );
          })}
        </div>
        <hr />
        <div className="row">
          {state.products.map((product) => {
            return (
              <div className="col-md-3" key={product.id}>
                <div
                  className="product-card border d-flex flex-column align-items-center justify-content-between"
                  style={{ minHeight: "300px" }}
                >
                  <img
                    src={BASE_URL + product.product_image}
                    style={{ height: "200px", width: "100%" }}
                  />
                  <h5>{product.title}</h5>
                  <h3 className="text-success">{product.price} Rs.</h3>
                  <button
                    onClick={() => {
                      navigate(`/view-description/${product.id}`);
                    }}
                    className="btn btn-warning text-white"
                    style={{ width: "100%" }}
                  >
                    View description
                  </button>
                </div>
              </div>
            );
          })}
        </div>
      </div>
    </>
  );
}
export default Home;
